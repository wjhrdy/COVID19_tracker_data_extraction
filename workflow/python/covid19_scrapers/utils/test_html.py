...
def test_url_to_soup():
    ...
    with utils.UTILS_WEB_CACHE.with_instance(cache):
        ...
        soup = utils.html.url_to_soup('http://fake')
...
def test_find_all_links():
  ...
  <body>
    ...
    with utils.http.UTILS_WEB_CACHE.with_instance(cache):
        session.add_response(content=content)
        links = utils.html.find_all_links('http://fake')
        assert links == ['link1', 'link2', 'link3', 'link4', ]

        session.add_response(content=content)
        links = utils.html.find_all_links('http://fake', '3')
        assert links == ['link3']

        session.add_response(content=content)
        links = utils.html.find_all_links('http://fake', re.compile('[34]'))
...
def test_table_to_dataframe():
  ...
  <tbody>
    ...
    df = utils.html.table_to_dataframe(BeautifulSoup(table,
                                                     features='lxml'))
    print(df)
